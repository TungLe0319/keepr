<template>
  <div class="modal-body position-relative bg-warning">
    ...
    <div class="row">
      <div class="col-md-6">
        <form
          @submit.prevent="editForm == true ? handleEdit() : handleSubmit()"
          class=""
        >
      ...
      >
        <div class="card rounded border-0 my-1">
          ...
          <img
            v-else
            src="https://wallpapercave.com/wp/wp1873707.jpg"
            alt=""
            title="keep"
            class="img-fluid rounded"
          />
          ...
          >
            <span>
              <h5 class="mb-0 fw-bold text-light ms-1 markoOne">
                {{ editable?.name }}
              </h5>
            </span>
          </div>
...
export default {
  ...
  setup(props) {
    const editable = ref({});
    const testForm = ref({
      img: "https://partners.wsj.com/xinhua/anhui/wp-content/uploads/sites/304/2018/05/xinhua-anhui1.jpg?resize=1200%2C720",
      description:
